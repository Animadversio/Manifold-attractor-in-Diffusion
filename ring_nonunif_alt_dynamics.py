...
import scipy.stats
from scipy.special import i0, i1, i0e, i1e, iv, ive
from scipy.integrate import odeint, solve_ivp
from tqdm import tqdm

# Note using i1e / i0e is more stable than i1 / i0
gamma = 0.5  # strength for non uniform bump
...
def dr_ode(sigma, r, theta):
    r_modify = r * R / sigma ** 2
    return r / sigma - R / sigma * (i1e(r_modify) - \
            gamma / 2 * np.cos(theta - psi_star) * (i0e(r_modify) + ive(2, r_modify))) / \
            (i0e(r_modify)- gamma * np.cos(theta - psi_star) * i1e(r_modify))


def dtheta_ode(sigma, r, theta):
    r_modify = r * R / sigma ** 2
    return - sigma / r ** 2 * gamma * np.sin(theta - psi_star) * i1e(r_modify) / \
            (i0e(r_modify) - gamma * np.cos(theta - psi_star) * i1e(r_modify))
...
figroot = r"/Users/binxuwang/Library/CloudStorage/OneDrive-HarvardUniversity/HaimDiffusionRNNProj/Ring_Nonunif"
# figdir = figroot + r"/dynamics_rinit"
figdir = figroot + r"/dynamics_rinit_noexp"
sigma_max = 100
sigma_min = 0.005
"""
...
"""
theta_dist = np.linspace(0, 2 * np.pi, 500, endpoint=False)
...
for r_init_norm in [100, 50, 25, 12.5, 6, 3, 1.5, 1, 0.5, 0.2, 0.1, 0.04]:
    ...
    plt.title(f"Radial Trajectories (VP-scaling) for init r={r_init_norm}\n psi*={psi_peak / np.pi:.2f}$\pi$ gamma={gamma} R={R} VP convention")
    saveallforms(figdir, f"radial_traj_rinit{r_init_norm:.1f}_sigmamax{sigma_max}_vp_expt")
    plt.show()
    #%% Plot the theta r trajectories
    plt.figure()
    for ysol in ysol_col[::2]:
        plt.plot(- np.log10(ysol.t[:]), ysol.y[-1, :], color="gray", alpha=0.2)
    plt.axhline(R, color="r", linestyle="--")
    # plt.xlim([sigma_min-0.2, sigma_max+0.2])
    plt.xlabel("t (sigma=exp(-t))")
    plt.ylabel("theta")
    plt.title(f"Angular Trajectories (VP-scaling) for init r={r_init_norm}\n psi*={psi_peak / np.pi:.2f}$\pi$ gamma={gamma} R={R} VP convention")
    saveallforms(figdir, f"angular_traj_rinit{r_init_norm:.1f}_sigmamax{sigma_max}_vp_expt")
    plt.show()
    ...
    for ysol in ysol_col[::2]:
        alpha_t = 1 / np.sqrt(1 + ysol.t[:]**2)
        plt.plot(ysol.y[0, :] * np.cos(ysol.y[-1, :]) * alpha_t,
                 ysol.y[0, :] * np.sin(ysol.y[-1, :]) * alpha_t, color="gray", alpha=0.2)
    # plot unit circle
    plt.plot(np.cos(np.linspace(0, 2 * np.pi, 100)),
             np.sin(np.linspace(0, 2 * np.pi, 100)), color="r", linestyle="--")
    plt.xlabel("x1 (alpha_t * x_1)")
    plt.ylabel("x2 (alpha_t * x_2)")
    plt.axis("image")
    plt.title(f"Planar Trajectories (VP-scaling) for init r={r_init_norm}\n psi*={psi_peak / np.pi:.2f}$\pi$ gamma={gamma} R={R}")
    saveallforms(figdir, f"planar_traj_rinit{r_init_norm:.1f}_sigmamax{sigma_max}_vp_expt")
    plt.show()
    #%%
    ttraj = ysol.t
